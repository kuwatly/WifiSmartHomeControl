...
#define DEMO_VERSION        "v3.00"		// Demo version
...
//ROM BYTE HTML_UP_ARROW1[]="99";
BYTE number[5]="-----"; // Assume that the value has been initialized
BYTE digit = 0;
int n = 0;
int i = 0;
...
//char dig = (char)(((int)'0')+i);
    //int higherDigit = ( ReadADC10(0) / 102 ) ;
    //number[0] = (char)(((int)'0') + (int) (higherDigit/10));//+((ReadADC10(0)/10)));
    //number[1] = (char)(((int)'0') + (int) (higherDigit%10));//+((ReadADC10(0)%10)));

    //number[0] = (char)(((int)'0') + (( ReadADC10(0) / 102 ) & 0xFF));
    //number[1] = (unsigned char) ((int)'0' +( (( ReadADC10(0) / 102 ) >> 4) & 0xFF));
    //number[1] = (( ReadADC10(0) / 102 ) >> 8) & 0xFF;
    //number[0] = num & 0x00FF;
    //number[1] = num & 0xFF00;
    //HTML_UP_ARROW1 = num;
    // Print the output
    ...
    //number[0] = (BYTE) ((int)'0' +( (( ReadADC10(0) / 10 ) >> 4) & 0xFF));
    ///number[1] = (BYTE) ((int)'0' +( ( ReadADC10(0) / 10 ) & 0xF));
    //number[0] = (BYTE) ((int)'0' +( (( ReadADC10(0) / 10 ) >> 4) & 0xFF));
    //number[1] = (BYTE) ((int)'0' +( ( ReadADC10(0) / 10 ) & 0xF));
     n = ReadADC10(0);
...
/*
    do {
       int digit = n % 10;
       number[i] = (BYTE) ((int)'0' + digit);
       n /= 10;
       i--;
    } while (n > 0 && i>0);
*/
    TCPPutString(sktHTTP, number);
    return;
}

//void HTTPPrint_TEMPDATA(void)
//{
//    char ipAddr[16];
//   // if(SW0_IO == 0 )
//    channel15 = 27 ;
//    sprintf(ipAddr,"%u",channel15);
